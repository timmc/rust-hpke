...
impl TestableKem for XWing {
    fn encaps_det(
        ...
        let ret = XWing::encap(&pk_recip, sender_keypair, &mut prng);
        prng.assert_done();
        ret
    }
}
...
#[test]
fn kat_test() {
    for file_name in ["test-vectors-5f503c5.json", "test-vectors-xwing.json"] {
        let file = File::open(file_name).unwrap();
        let tvs: Vec<MainTestVector> = serde_json::from_reader(file).unwrap();

        for tv in tvs.into_iter() {
            // Ignore everything that doesn't use X25519, P256, P384 or P521,
            // or X-Wing, since that's all we support right now
